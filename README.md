# 🥕 NeuroDivertidos: La Fragua de los Morros Soberanos 🤡⚔️
**Status:** PROD_READY | **Deploy:** GitHub Pages | **Vibe:** Laugh & Fight (Savage Pedagogy)

**NeuroDivertidos** no es solo una colección de juegos; es el arsenal pedagógico de la **Slime Legion** para morros que ven el mundo diferente. Aquí reímos, peleamos contra el aburrimiento y dominamos la ciencia con herramientas de alta fidelidad.

## 🎮 El Arsenal de Juegos

### 1. [Mini Zanahoria: El Origen](./mini-zanahoria/)
Entrenamiento de **atención conjunta** y **resonancia emocional**. Diseñado para que los morros con TEA/TDAH dominen el enfoque mientras se ríen de una zanahoria escurridiza.

### 2. [Abecedario Mágico (V17)](./magic-alphabet/)
Caligrafía terapéutica con motor vectorial. Porque escribir bien es el primer paso para hackear el sistema. Cero aburrimiento, puro refuerzo positivo.

### 3. [Slime Evolution](./slime-evolution/)
Sistema de evolución de slimes con sprites empaquetados en spritesheets optimizados para el navegador.

## 🏗️ Producción Autónoma (Nodo Beta / NixOS)
Estos juegos son scouteados por **Odysseus** desde la ciencia de **OpenAlex** y forjados por la Legion en las sombras de NixOS. Si un juego no es lo suficientemente "NeuroDivertido", la Legion lo borra y empieza de nuevo.

---
*Protocolo MAIAH: Aprender es una pelea que se gana riendo.* 🫡🚀🩺💎✨🥕🤡⚔️
